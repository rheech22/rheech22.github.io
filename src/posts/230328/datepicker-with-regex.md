...
웹에서 커스텀한 Date Picker를 보여주고 싶다면, 말 그대로 커스텀한 구성 요소를 직접 만들거나 라이브러리를 활용해야 한다. MUI와 같은 라이브러리는 연, 월, 일을 구분하는 구분자를 자동으로 추가하고 유효성도 잘 체크한다. 하지만 내게 필요했던 range 기능은 유료여서 채택할 수 없었다. 결국 `react-datepicker`를 골랐는데 이 라이브러리가 제공하는 인풋에는 아무 문자나 입력 가능하다는 아쉬움이 있었다. 유효하지 않은 입력을 제한하고 날짜 형식을 자동으로 잡아줄 필요가 있었다. 이번 글에서는 그 목적을 달성하기 위해 정규 표현식을 공부했던 내용을 남겼다. 최대한 간단한 모양의 HTML과 스크립트로 예제를 구성했다.
...
그러나 나는 사실 8자리 모두 입력하지 않더라도 필요한 시점에 점이 찍히길 원한다. 그러기 위해선 수량자를 변경해야 한다. 각 그룹이 0개의 숫자부터 캡쳐링할 수 있도록 수량자를 조절한다. 하지만 이렇게만 하면 `1..`과 같이 점이 연속으로 찍히고 그 다음의 입력은 마지막 자리에 붙게 된다.
...
아래와 같이 포맷을 바꿔주기 전에 숫자를 제외한 문자를 제거하면 최종적으로 원하는 위치에 점을 찍어준다.
...
다만 날짜를 모두 지우거나 종료 날짜가 없음에도 나타나는 하이픈(-)을 지우고 싶다. 정규식을 하나 더 추가한다. 부정 전방 탐색을 사용해서 뒤에 `숫자`, 또는 `공백 + 숫자`가 오지 않는 하이픈에 대한 매칭을 수행한다. 하이픈의 앞뒤에는 공백 문자가 0개 이상 올 수 있다.
...
- 전방 탐색(positive lookahead)은 정규 표현식에서 일치하는 부분 문자열을 제외한 부분에 대해 매칭을 수행하는 것을 말한다. 예를 들어, `q(?=u)`는 `u` 오른쪽에 둔 `q`를 매칭한다. 이 정규식은 `quiet`에서 `q`에 대해 매칭을 수행하지만, `Qantas`와 같은 문자열에서는 매칭을 수행하지 않는다.
...
다만 이렇게 여러 정규식을 추가하는건 좋은 방법이 아닌 것 같다. 여전히 커버하지 못하는 엣지 케이스가 있고, 이를 정규식 보다는 인풋의 입력 영역을 나눈다던지, 좀 더 확실한 방법으로 해결할 필요가 있다고 느꼈다. 아마도 MUI의 `<DateField />`와 같은 모습이라면 좋을 것 같은데 나중에 더 나은 방법을 찾게 되면 기록을 남기려고 한다.
